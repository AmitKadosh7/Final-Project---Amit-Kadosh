%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Golden Model for Un-weighted shrinkage
% SLT Denoise, produce the set of all test images
% with SSIM and scaling for various STD levels, modified 27/8/2009
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;
clear;
close all;
...
for i=1:Num_of_Test_Images % test images counter
    X=imread(Test_Image{i}); % Training Image
    [DIM1,DIM2]=size(X);
    for n=1:Noise_Realizations % Noise realization counter
        % Create Noisy Image
        Y=double(X)+ randn(DIM1,DIM2)*test_sigma; % Noisy Training Image
        Xe=0;
    ...
    Y = uint8(Y);
    figure(i); subplot(1,3,1); imshow(X); title('Original');
    subplot(1,3,2); imshow(Y); title('Noisy');
    subplot(1,3,3); imshow(Xe); title('Denoised');
end
