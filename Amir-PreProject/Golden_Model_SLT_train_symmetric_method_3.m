%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Golden Model for Un-weighted shrinkage Training 27/8/2009
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% single precision, multiple patches, with out of range treatment
% Sigma=10, lamda=(0.001*DIM*DIM/M)^2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:K
    ...
    if k>0
        % Symmetric MF boundaries
        New_Min=0;
        New_Max=0.98*max(abs(Range(k,2)),abs(Range(k,1))); % Use 98% of full range
        Range(k,1)=New_Min;
        Range(k,2)=New_Max;
    end
end
...
for r=1:R
    ...
    for k=1:K
        ...
        YB_tmp=squeeze(YB(k,:,:));
        % Use symmetric MF
        [Sy q(:,k) hy]=Sq3(YB_tmp(:),M,Range(k,1),Range(k,2));
        ...
        XB(k,:,:)=tmp_conv_X;
        XB_tmp=squeeze(XB(k,:,:));
        [Sx q(:,k) hx]=Sq3(XB_tmp(:),M,Range(k,1),Range(k,2));
        L_Trans_X((k-1)*(M+1)+1:k*(M+1)) = single(full(Sy)')*XB_tmp(:);
        % calc out-of-range subband k componenet of L_Transpose
